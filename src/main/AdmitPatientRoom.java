...
import dao.AdmitPatientRoomDao;
import dao.Connector;
import net.proteanit.sql.DbUtils;

@SuppressWarnings("serial")
public class AdmitPatientRoom extends JInternalFrame implements ActionListener {

	Dimension d = Toolkit.getDefaultToolkit().getScreenSize();
	JLabel lblAdmitId, lblPatientId, lblPatientName, lblGender, lblBloodGroup, lblDisease, lblAdmitDate, lblRoomNo,
	...
	JComboBox<String> comboRoomNo;
	JButton butNew, butSave, butUpdate, butDelete, butGetData, butGo, butGoo;
	JTable table;
	String heading[] = { "Admit ID", "Patient ID", "Disease", "Room No", "Admit Date", "Doctor ID", "Remarks" };
	...
	JPanel admitInfo() {
		...
		c.gridy = 7;
		c.insets = new Insets(10, 10, 10, 10);
		comboRoomNo.setEnabled(false);
		pan.add(comboRoomNo, c);
	...
	JPanel panAdd() {
		...
		pan.add(admitInfo(), BorderLayout.CENTER);
	...
	JScrollPane scroll() {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent evt) {
				...
				String sql = "select * from admitpatient_room where AdmitID='" + tableClick + "'";
				try {
					...
					if (rs.next()) {
						...
						comboRoomNo.setSelectedItem(roomNo);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == butNew) {
			...
			String roomNo = String.valueOf(comboRoomNo.getSelectedItem());
			String doctorId = txtDoctorId.getText();
			String remarks = txtRemarks.getText();

			AdmitPatientRoomBean b = AdmitPatientRoomDao.exist(id);
			if (b.equals(id)) {
				return;
			}

			AdmitPatientRoomBean bean = new AdmitPatientRoomBean(id, patientId, disease, date, roomNo, doctorId,
					remarks);
			int status = AdmitPatientRoomDao.save(bean);
			if (status > 0) {
			...
			AdmitPatientRoomBean bean = new AdmitPatientRoomBean(id, patientId, disease, date, roomNo, doctorId,
					remarks);
			int status = AdmitPatientRoomDao.update(bean);
			if (status > 0) {
	...
	void get_Data() {
		Connection con = Connector.ConnectDB();
		String sql = "select * from admitpatient_room";
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
