...
import dao.DischargePatientWardDao;
import net.proteanit.sql.DbUtils;

@SuppressWarnings("serial")
public class DischargePatientWard extends JInternalFrame implements ActionListener {

	Dimension d = Toolkit.getDefaultToolkit().getScreenSize();
	JLabel lblDischargeId, lblAdmitId, lblPatientId, lblPatientName, lblGender, lblBloodGroup, lblDisease, lblAdmitDate,
			lblWardName, lblDoctorId, lblDoctorName, lblDischargeDate, lblRemarks;
	JTextField txtDischargeId, txtAdmitId, txtPatientId, txtPatientName, txtGender, txtBloodGroup, txtDisease,
			txtAdmitDate, txtWardName, txtDoctorId, txtDoctorName, txtDischargeDate;
	...
	JPanel dischargeInfo() {
		...
		pan.setBorder(BorderFactory.createTitledBorder(null, "Patient Discharge Info", TitledBorder.LEFT,
				TitledBorder.TOP, new Font("Cambria", Font.PLAIN, 15), Color.BLACK));
	...
	JScrollPane scroll() {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent evt) {
				...
				String sql = "select * from dischargepatient_ward where ID='" + tableClick + "'";
				try {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == butNew) {
			...
			DischargePatientWardBean b = DischargePatientWardDao.exist(dischargeId);
			if (b.equals(dischargeId)) {
				return;
			}

			DischargePatientWardBean bean = new DischargePatientWardBean(dischargeId, admitId, dischargeDate, remarks);
			int status = DischargePatientWardDao.save(bean);
			if (status > 0) {
			...
			DischargePatientWardBean bean = new DischargePatientWardBean(dischargeId, admitId, dischargeDate, remarks);
			int status = DischargePatientWardDao.update(bean);
			if (status > 0) {
			...
			if (status == 0) {
				...
				DischargePatientWardDao.delete(id);
