...
import dao.BillingPatientWardDao;
import dao.Connector;
import net.proteanit.sql.DbUtils;

@SuppressWarnings("serial")
public class BillingPatientWard extends JInternalFrame implements ActionListener {

	JLabel lblBillNo, lblDischargeId, lblAdmitId, lblPatientId, lblPatientName, lblGender, lblBloodGroup, lblDisease,
			lblAdmitDate, lblWardName, lblDoctorId, lblDoctorName, lblDischargeDate, lblBedCharges, lblNoOfDays,
			lblTotalBedCharges, lblServiceCharges, lblBillingDate, lblPaymentMode, lblPaymentModeDetails,
	...
	JTextField txtBillNo, txtDischargeId, txtAdmitId, txtPatientId, txtPatientName, txtGender, txtBloodGroup,
			txtDisease, txtAdmitDate, txtWardName, txtDoctorId, txtDoctorName, txtDischargeDate, txtBedCharges,
			txtNoOfDays, txtTotalBedCharges, txtServiceCharges, txtBillingDate, txtPaymentModeDetails, txtTotalCharges,
	...
	JTable table;
	String[] headings = { "Bill No", "Discharge ID", "Billing Date", "Bed Charges", "No.of Days", "TotalBedCharges",
			"ServiceCharges", "TotalCharges", "PaymentMode", "PaymentModeDetails", "ChargesPaid", "DueCharges" };
	Dimension d = Toolkit.getDefaultToolkit().getScreenSize();

	public BillingPatientWard() {
	...
	JPanel patientInfo() {
		...
		txtWardName = new JTextField();
		c.anchor = GridBagConstraints.WEST;
		c.fill = GridBagConstraints.BOTH;
		c.gridx = 1;
		c.gridy = 6;
		c.insets = new Insets(5, 5, 5, 5);
		txtWardName.setEnabled(false);
	...
	JPanel paymentDetails() {
		...
		lblBedCharges = new JLabel("Bed Charges");
		c.anchor = GridBagConstraints.WEST;
		c.fill = GridBagConstraints.BOTH;
		c.gridx = 0;
		c.gridy = 0;
		c.insets = new Insets(5, 5, 5, 5);
		pan.add(lblBedCharges, c);

		txtBedCharges = new JTextField();
		c.anchor = GridBagConstraints.WEST;
		c.fill = GridBagConstraints.BOTH;
		c.gridx = 1;
		c.gridy = 0;
		c.insets = new Insets(5, 5, 5, 5);
		txtBedCharges.setEnabled(false);
		pan.add(txtBedCharges, c);
		...
		lblTotalBedCharges = new JLabel("Total Bed Charges");
		c.anchor = GridBagConstraints.WEST;
		c.fill = GridBagConstraints.BOTH;
		c.gridx = 4;
		c.gridy = 0;
		c.insets = new Insets(5, 5, 5, 5);
		pan.add(lblTotalBedCharges, c);

		txtTotalBedCharges = new JTextField(10);
		c.anchor = GridBagConstraints.WEST;
		c.fill = GridBagConstraints.BOTH;
		c.gridx = 5;
		c.gridy = 0;
		c.insets = new Insets(5, 5, 5, 5);
		txtTotalBedCharges.setEnabled(false);
		pan.add(txtTotalBedCharges, c);
	...
	JScrollPane scroll() {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent evt) {
				...
				String sql = "select * from bill_ward where BillNo='" + tableClick + "'";
				try {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == butNew) {
			...
			double bedCharges = Double.parseDouble(txtBedCharges.getText());
			int noOfDays = Integer.parseInt(txtNoOfDays.getText());
			double totalBedCharges = Double.parseDouble(txtTotalBedCharges.getText());
			...
			BillingPatientWardBean b = BillingPatientWardDao.exist(billNo);
			if (b.equals(billNo)) {
				return;
			}

			BillingPatientWardBean bean = new BillingPatientWardBean(billNo, dischargeId, billingDate, bedCharges,
					noOfDays, totalBedCharges, serviceCharges, totalCharges, paymentMode, paymentModeDetails,
					chargesPaid, dueCharges);
			int status = BillingPatientWardDao.save(bean);
			if (status > 0) {
			...
			BillingPatientWardBean bean = new BillingPatientWardBean(billNo, dischargeId, billingDate, bedCharges,
					noOfDays, totalBedCharges, serviceCharges, totalCharges, paymentMode, paymentModeDetails,
					chargesPaid, dueCharges);
			int status = BillingPatientWardDao.update(bean);
			if (status > 0) {
			...
			if (status == 0) {
				int billNo = Integer.parseInt(txtBillNo.getText());
				BillingPatientWardDao.delete(billNo);
			...
			getWardData(admit);
			getBedCharges(txtWardName.getText());
	...
	public double totalCharges() {
		...
		double totalBedCharges = Double.parseDouble(txtTotalBedCharges.getText());
		double serviceCharges = Double.parseDouble(txtServiceCharges.getText());
		totalCharges = totalBedCharges + serviceCharges;
	...
	public double totalBedCharges() {
		double totalBedCharges = 0;
		double days = Double.parseDouble(txtNoOfDays.getText());
		double bedCharges = Double.parseDouble(txtBedCharges.getText());
		totalBedCharges = bedCharges * days;
		String cost = Double.toString(totalBedCharges);
		txtTotalBedCharges.setText(cost);
	...
	public void getBedCharges(String wardName) {
		Connection con = Connector.ConnectDB();
		String sql = "select Charges from ward where wardname='" + wardName + "'";
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				String charge = Integer.toString(rs.getInt("Charges"));
				txtBedCharges.setText(charge);
			}
		} catch (SQLException e) {
			JOptionPane.showMessageDialog(this, e);
		}
	...
	public void getDischargeData(int dischargeId) {
		Connection con = Connector.ConnectDB();
		String sql = "select DischargeDate from dischargepatient_ward where ID='" + dischargeId + "'";
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
	...
	public void getWardData(int id) {
		try {
			Connection con = Connector.ConnectDB();
			String sql = "select Disease,Wardname,AdmitDate from admitpatient_ward where AdmitID='" + id + "'";
			PreparedStatement ps = con.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				String disease = rs.getString("Disease");
				txtDisease.setText(disease);
				String room = rs.getString("Wardname");
				txtWardName.setText(room);
	...
	public void get_Data() {
		Connection con = Connector.ConnectDB();
		String sql = "select * from bill_ward";
		try {
			PreparedStatement ps = con.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
